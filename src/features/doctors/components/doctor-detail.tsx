...
import { ArrowLeft, Star, Clock, GraduationCap, Calendar, Loader2, UserRound } from 'lucide-react'
...
export default function DoctorDetail() {
  ...
  const { data: doctor, isLoading } = useQuery(doctorQueries.detail(Number(doctorId)))
          ...
          <div className="relative bg-gradient-to-br from-blue-600 via-blue-500 to-cyan-500 px-8 pt-16 pb-20 text-white">
            <div className="flex flex-col md:flex-row items-center md:items-end gap-6">
              ...
              <div className="w-32 h-32 rounded-2xl overflow-hidden bg-white/20 ring-4 ring-white/30 shadow-xl shrink-0">
                {doctor.image ? (
                  <img src={getDoctorImageUrl(doctor.id)} alt={doctor.name} className="w-full h-full object-cover" />
                ) : (
                ...
                <div className="flex items-center gap-3 justify-center md:justify-start mb-2">
                  ...
                  {doctor.rating && (
                    <span className="flex items-center gap-1 text-sm bg-white/20 px-3 py-1 rounded-full">
                      ...
                      {doctor.rating}
                      {doctor.reviews && <span className="opacity-70">({doctor.reviews})</span>}
                    </span>
                  )}
                </div>

                <h1 className="text-3xl md:text-4xl font-bold mb-1">{doctor.name}</h1>
                <p className="text-blue-100 text-lg">
                  {doctor.title ?? doctor.department ?? 'Medical Specialist'}
                </p>
                {doctor.experience && (
                  <p className="text-blue-200 text-sm mt-1">{doctor.experience} of experience</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="p-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-slate-50 rounded-xl p-5 space-y-4">
                <h3 className="font-bold text-slate-800 text-sm uppercase tracking-wider">Quick Info</h3>
                <div className="space-y-3 text-sm">
                  ...
                  {doctor.reviews && (
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-medium text-slate-800">{doctor.reviews}</span>
                    </div>
                  )}
                  <div className="flex items-center justify-between">
              ...
              {/* Schedule Cards */}
              {doctor.schedule && (() => {
                const isDualType = doctor.type.includes('consultant') && doctor.type.includes('outdoor')
                ...
                const consultantSchedule = parsed.consultant ? parsed.consultant.split('\n') : []
                const outdoorSchedule = parsed.outdoor ? parsed.outdoor.split('\n') : []
                const generalSchedule = parsed.general ? parsed.general.split('\n') : []
