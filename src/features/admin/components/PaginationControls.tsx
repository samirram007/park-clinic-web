...
export const PaginationControls: React.FC<PaginationControlsProps> = ({
  ...
  return (
    <div
      className={`flex items-center gap-1.5 ${compact ? '' : 'justify-center py-4 border-t border-slate-100 my-4'}`}
    >
      ...
      {pageNumbers.map((item, idx) =>
        item === 'ellipsis' ? (
          <span
            key={`ellipsis-${idx}`}
            className="px-1 text-slate-300 select-none text-xs"
          >
            …
          </span>
        ) : (
