...
import { adminCareerApplicationService } from '../data/api'
import type { CareerApplication } from '../data/schema'
...
import type { JobPost } from '../data/schema'
...
export const CareerApplications: React.FC = () => {
  ...
  const applications: CareerApplication[] = data?.data ?? []
  ...
  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <Button variant="ghost" size="icon" onClick={() => queryClient.invalidateQueries({ queryKey: ['admin-career-applications'] })} title="Refresh">
          <RefreshCw size={16} />
        </Button>
      </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 p-6 relative">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3 pb-4 border-b border-slate-100">
                ...
                <div>
                  <h3 className="font-semibold text-slate-900">{selectedApp.full_name}</h3>
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-sm text-slate-500">{selectedApp.position}</span>
                    {activeJobTitles.has(selectedApp.position.toLowerCase().trim()) && (
              ...
              <div className="grid grid-cols-1 gap-3">
                ...
                <div className="flex items-center gap-3 text-sm">
                  ...
                  <a href={`tel:${selectedApp.phone}`} className="text-slate-700">{selectedApp.phone}</a>
                </div>
                <div className="flex items-center gap-3 text-sm">
                ...
                <div className="flex items-center gap-3 text-sm">
                  ...
                  <span className="text-slate-500">{new Date(selectedApp.created_at ?? '').toLocaleDateString()}</span>
                </div>
              </div>

              {selectedApp.message && (
                <div className="pt-4 border-t border-slate-100">
                  <label className="text-sm font-medium text-slate-700 mb-1 block">Cover Letter / Message</label>
                  <p className="text-sm text-slate-600 whitespace-pre-wrap bg-slate-50 rounded-lg p-3">{selectedApp.message}</p>
                </div>
              )}
            ...
            <div className="flex justify-end gap-3 pt-6 border-t border-slate-100 mt-6">
              ...
              <Button
                ...
                size="sm"
                onClick={() => {
                  setDeleteTarget({ id: selectedApp.id, name: selectedApp.full_name })
                  setSelectedApp(null)
                }}
                className="gap-2"
              >
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(open) => { if (!open) setDeleteTarget(null) }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete <strong>{deleteTarget?.name}</strong>'s application?
              <br /><br />
              The uploaded resume will also be permanently deleted. This cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <div className="space-y-2">
        {isLoading ? (
          ...
          <div className="text-center py-20 text-slate-400">
            ...
            <p className="text-sm">Applications from the career page will appear here.</p>
          </div>
        ) : (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <h3 className="font-semibold text-slate-900 truncate max-w-sm">{app.full_name}</h3>
                      <div className="flex items-center gap-2 mt-0.5">
                        {activeJobTitles.has(app.position.toLowerCase().trim()) && (
                          <span className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 font-medium" title="This position is an active job posting">
                            <BadgeCheck size={11} />
                            Applied
                          </span>
                        )}
                  ...
                  <div className="flex items-center gap-2 mt-4 pt-3 border-t border-slate-100">
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={(e) => { e.stopPropagation(); setDeleteTarget({ id: app.id, name: app.full_name }) }}
