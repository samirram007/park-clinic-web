...
import { Inbox, Mail, MailOpen, RefreshCw, Search, Star } from 'lucide-react'
...
export const ContactMessageComponent: React.FC = () => {
  ...
  const [filter, setFilter] = useState<'all' | 'read' | 'unread' | 'important'>(
    'unread',
  )
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: () =>
      contactMessageService.getMessages({
        page,
        per_page: perPage,
        search,
  ...
  const { data: allData } = useQuery({
    ...
    queryFn: () =>
      contactMessageService.getMessages({ status: 'all', per_page: 1 }),
  })

  const { data: unreadData } = useQuery({
    queryKey: ['contact-messages-count-unread'],
    queryFn: () =>
      contactMessageService.getMessages({ status: 'unread', per_page: 1 }),
  })

  const { data: readData } = useQuery({
    queryKey: ['contact-messages-count-read'],
    queryFn: () =>
      contactMessageService.getMessages({ status: 'read', per_page: 1 }),
  })

  const { data: importantData } = useQuery({
    queryKey: ['contact-messages-count-important'],
    queryFn: () =>
      contactMessageService.getMessages({ important: true, per_page: 1 }),
  })
          ...
          >
            <TabsList className="bg-slate-100/50 p-1">
              ...
              >
                ...
                {unreadData?.meta?.total !== undefined
                  ? `(${unreadData.meta.total})`
                  : '(0)'}
              </TabsTrigger>
              <TabsTrigger
                value="read"
              ...
              >
                ...
                {importantData?.meta?.total !== undefined
                  ? `(${importantData.meta.total})`
                  : '(0)'}
              </TabsTrigger>
            </TabsList>
          </Tabs>
