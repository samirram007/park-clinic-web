...
export const MessageListItem: React.FC<MessageListItemProps> = ({
  ...
  return (
    ...
    >
      <div
        className={`p-1.5 rounded-full ${!msg.readAt ? 'bg-blue-100 text-blue-600' : 'bg-slate-100 text-slate-400'}`}
      >
      ...
      >
        <div className="min-w-0 truncate">
          <p
            className={`text-sm font-semibold truncate ${!msg.readAt ? 'text-slate-900' : 'text-slate-400'}`}
          >
            {msg.name}
          </p>
        ...
        <div className="grid grid-cols-[auto_auto_1fr] items-center gap-2 min-w-0">
          ...
          <span
            className={`text-sm truncate ${!msg.readAt ? 'text-slate-500' : 'text-slate-400'}`}
          >
            {msg.message}
          </span>
        </div>
