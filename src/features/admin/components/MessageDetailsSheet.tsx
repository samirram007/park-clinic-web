      ...
      <SheetContent className="w-[400px] sm:w-[600px] p-0 flex flex-col h-full">
            ...
            <div className="space-y-4">
              <div className="bg-blue-50 p-6 rounded-xl border border-blue-100 shadow-sm space-y-2">
                <div className="flex justify-between items-center border-b border-blue-100 pb-2 mb-2">
                  <Label className="text-blue-900 font-bold uppercase text-[10px] tracking-wider">Your Reply</Label>
                  <span className="text-[10px] text-blue-500 font-medium">
                    {selectedMessage.replyAt && new Date(selectedMessage.replyAt).toLocaleString()}
                  </span>
                </div>
            ...
            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-4">
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <Label htmlFor="reply" className="text-slate-900 font-bold uppercase text-[10px] tracking-wider">
              ...
              >
                {isReplying ? 'Sending...' : isEditing ? 'Update & Resend' : 'Send Reply'}
              </Button>
            </div>
          )}
        </div>
        ...
        <div className="p-6 border-t flex justify-between items-center bg-white">
          <div className="flex items-center gap-2">
            {selectedMessage.readAt && <Badge variant="secondary" className="px-3 rounded-full">Read</Badge>}
            <Button 
              variant="ghost" 
              size="sm" 
            ...
            >
              <Star size={16} fill={selectedMessage.isImportant ? 'currentColor' : 'none'} />
              {selectedMessage.isImportant ? 'Important' : 'Mark Important'}
            </Button>
          </div>
          <Button variant="outline" onClick={() => onMarkAsUnread(selectedMessage.id)}>
            Mark as Unread
          </Button>
        </div>
      </SheetContent>
