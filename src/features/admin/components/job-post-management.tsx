...
export const JobPostManagement: React.FC = () => {
  ...
  const { data: activeCountData } = useQuery({
    ...
    queryFn: () => adminJobPostService.getPosts({ per_page: 1, status: 'active' }),
  })

  const { data: expiredCountData } = useQuery({
    queryKey: ['admin-job-posts-count-expired'],
    queryFn: () => adminJobPostService.getPosts({ per_page: 1, status: 'expired' }),
  })

  const { data: inactiveCountData } = useQuery({
    queryKey: ['admin-job-posts-count-inactive'],
    queryFn: () => adminJobPostService.getPosts({ per_page: 1, status: 'inactive' }),
  })
  ...
  const [deactivateTarget, setDeactivateTarget] = useState<{ id: number; title: string } | null>(null)
  ...
  const posts: JobPost[] = data?.data ?? []
  const meta = data?.meta

  const createMutation = useMutation({
    mutationFn: (payload: Record<string, unknown>) => adminJobPostService.createPost(payload),
    onSuccess: () => {
  ...
  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-2xl font-bold tracking-tight text-slate-900">Manage Job Posts</h1>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 bg-white p-2 rounded-xl border border-slate-200 shadow-sm mb-6">
        <Tabs value={statusFilter} onValueChange={(v) => { setStatusFilter(v); setPage(1) }} className="w-full sm:w-auto">
          <TabsList className="bg-slate-100/50 p-1">
            ...
            >
              ...
              <span className="font-semibold">All</span> {allCountData?.meta?.total !== undefined ? `(${allCountData.meta.total})` : ''}
            </TabsTrigger>
            <TabsTrigger
              value="active"
            ...
            >
              ...
              <span className="font-semibold">Active</span> {activeCountData?.meta?.total !== undefined ? `(${activeCountData.meta.total})` : ''}
            </TabsTrigger>
            <TabsTrigger
              value="inactive"
            ...
            >
              ...
              <span className="font-semibold">Inactive</span> {inactiveCountData?.meta?.total !== undefined ? `(${inactiveCountData.meta.total})` : ''}
            </TabsTrigger>
            <TabsTrigger
              value="expired"
            ...
            >
              ...
              <span className="font-semibold">Expired</span> {expiredCountData?.meta?.total !== undefined ? `(${expiredCountData.meta.total})` : ''}
            </TabsTrigger>
          </TabsList>
        </Tabs>
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          <Button variant="ghost" size="icon" onClick={() => {
            queryClient.invalidateQueries({ queryKey: ['admin-job-posts'] })
            queryClient.invalidateQueries({ queryKey: ['admin-job-posts-count-all'] })
            queryClient.invalidateQueries({ queryKey: ['admin-job-posts-count-active'] })
            queryClient.invalidateQueries({ queryKey: ['admin-job-posts-count-expired'] })
            queryClient.invalidateQueries({ queryKey: ['admin-job-posts-count-inactive'] })
          }} title="Refresh">
            <RefreshCw size={16} />
          </Button>
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 p-6 relative">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                ...
                <Input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} required placeholder="e.g. Staff Nurse" />
              </div>

              {/* Description */}
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Description *</label>
                <Textarea rows={6} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} required placeholder="Describe the role, responsibilities, qualifications..." />
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {/* Apply Duration */}
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Apply By (Deadline)</label>
                  <Input
                    type="date"
                    value={form.apply_duration}
                    onChange={(e) => setForm({ ...form, apply_duration: e.target.value })}
                  />
                </div>
              </div>
              ...
              <div className="flex items-center justify-end gap-3 pt-2">
                ...
                <Button type="submit" disabled={isPending || !form.title || !form.description} className="gap-2">
      ...
      <div className="space-y-3">
        {isLoading ? (
          <div className="grid gap-3">
            {[1, 2, 3].map((i) => (
              <div key={i} className="flex items-center gap-4 bg-white rounded-xl border border-slate-200 p-4">
            ...
            >
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <h3 className="font-semibold text-slate-900 truncate max-w-md">{post.title}</h3>
                    ...
                    <span className={`text-xs px-2.5 py-1 rounded-full font-medium shrink-0 shadow-sm ${
                      post.is_active
                  ...
                  {/* Description */}
                  <p className="text-sm text-slate-500 mt-2.5 leading-relaxed line-clamp-3">{post.description}</p>
                  ...
                  <div className="flex flex-wrap items-center gap-x-5 gap-y-2 mt-3 text-xs text-slate-400">
                    {post.apply_duration && (
                      <span className="inline-flex items-center gap-1.5 bg-amber-50 text-amber-700 px-2.5 py-1 rounded-md font-medium">
                        <Calendar size={12} />
                        Deadline: {new Date(post.apply_duration).toLocaleDateString()}
                      </span>
                    )}
                    ...
                    <span className="inline-flex items-center gap-1.5 text-blue-600 font-medium">
                      <Briefcase size={12} />
                      {post.applications_count ?? 0} applicant{(post.applications_count ?? 0) !== 1 ? 's' : ''}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-2 mt-4 pt-3 border-t border-slate-100">
                    <button
                      type="button"
                      onClick={() => handleToggleStatus(post.id, post.is_active, post.title)}
                      disabled={toggleStatusMutation.isPending}
                      title={post.is_active ? 'Deactivate this job post' : 'Activate this job post'}
                    ...
                    >
                      <span
                        className={`inline-block h-4 w-4 transform rounded-full bg-white shadow-sm ring-0 transition-transform duration-200 ${
                          post.is_active ? 'translate-x-[18px]' : 'translate-x-[2px]'
                        }`}
                      />
                    </button>
                    <div className="ml-auto flex items-center gap-1">
                      <Button variant="ghost" size="sm" onClick={() => handleEdit(post)} className="gap-1.5 text-slate-600 hover:text-blue-700 hover:bg-blue-50">
                        <Pencil size={14} />
                        Edit
                      </Button>
                      <Button variant="ghost" size="sm" onClick={() => setDeleteTarget({ id: post.id, title: post.title })} className="gap-1.5 text-red-500 hover:text-red-700 hover:bg-red-50">
                        <Trash2 size={14} />
                        Delete
                      </Button>
                    </div>
