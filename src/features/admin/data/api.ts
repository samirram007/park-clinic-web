...
import type { GetMessagesParams, GetDoctorsParams, GetJobPostsParams, GetCareerApplicationsParams } from './schema';
...
export const adminDoctorService = {
    ...
    updateDoctor: async ({ id, payload }: { id: number; payload: Record<string, unknown> }) => {
        const fd = buildDoctorFormData(payload);
...
export const adminJobPostService = {
    ...
    updatePost: async ({ id, payload }: { id: number; payload: Record<string, unknown> }) => {
